...
意味検索・キャッシュ照合はホットパスであり性能要件があるため、実装言語は **C++**(初期Pythonプロトタイプは `poc/python_prototype/` に退避)。

## 2. 構成とモジュール

| モジュール | 実装 | 設計メモとの対応 |
|---|---|---|
| Embedding | `IEmbedder`。既定 `MockEmbedder`(文字n-gramハッシュ→512次元→L2正規化、決定論的・モデル不要)。オプション `OnnxEmbedder`(ONNX Runtime、MiniLM級モデル読込の拡張点) | §1: MPNet級モデル推奨 → 実運用はONNX経路。PoCは依存ゼロ優先 |
| 意味検索 | 全エントリとの内積(正規化済みなのでコサイン類似度)を brute-force 走査 | PoC規模でO(n)十分。faiss/HNSWへの差し替え拡張点 |
| しきい値 | `kLocalThreshold=0.80` / `kSharedThreshold=0.90` | §1(MeanCache τ≈0.83)・§2脅威A(共有は精度優先で0.9+) |
...
| 詐称防止 | `ISigner`。既定 `DummySigner`(sha256鍵付きMAC=プレースホルダ)、`POC_USE_SODIUM=ON` で libsodium **Ed25519** | §4「署名=誰が言ったかの固定」 |
| Agent | `IAgent`。既定 `MockAgent`(固定回答、ネット不要)。実LLM(Claude等)は同IFへの差し込み拡張点(HTTP依存を必須にしない) | Agent層の抽象化 |
...
- **既定構成は外部依存ゼロ**(CMake + C++17 コンパイラのみ)。SHA-256 は自前単一ヘッダ、JSON は nlohmann/json 単一ヘッダを `vendor/` に同梱。
- 重量級依存は CMake オプションで任意有効化:
  - `POC_USE_SODIUM=ON` → Ed25519 実署名(libsodium)
  - `POC_USE_ONNX=ON` → 実Embedding経路(ONNX Runtime。トークナイザ統合は今後)
- どのオプションが無くても Mock 経路で通しビルド・実行できる。
...
.\build\Release\semantic_cache_poc.exe     # Windows/MSVC
```

詳細は [`poc/README.md`](../poc/README.md)。

## 5. 動作確認結果(Mock経路 / Windows 11 + MSVC 19.51)
...
2. `DummySigner` は公開検証不可の MAC。署名フローの実証用であり、詐称防止の実効性は `POC_USE_SODIUM=ON` で得る
3. 揮発性 L0 のみ。permanent 昇格・事実トリプル分解(案4)・L2 Agent自己申告は未実装
4. TTL失効・witness・評判・revocation・regurgitationフィルタは P2P フェーズの課題
5. 検索は線形走査。大規模化時は faiss/HNSW + PCA圧縮(§1のMeanCache知見)に差し替え

## 7. 次のステップ候補

- ONNX経路のトークナイザ統合(tokenizers-cpp + multilingual-MiniLM)で言い換えヒットを実証
- libsodium 同梱ビルドを既定化し DummySigner を排除
