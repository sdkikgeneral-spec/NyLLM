...
- **[B] S2.5 エントリ形式の確定** — **設計完了・実装待ち(承認後 poc-core-dev)**。確定設計は [S2.5_エントリ形式設計.md](./S2.5_エントリ形式設計.md) を参照(詳細な経緯は下記§5)。不変コアをPublicまで見据えてフル確定し、可変状態は空スロットを型で宣言する。`poc/` 上・単一ノードのまま。
- **[C] `src/core`(Rust)への昇格 = Company Phase1本体** — 背骨(trait 3本/エントリ形式の不変コア/triples/volatility/共有ゲート/検索)を移植。**S3相当を"社内版"に縮約**(多ノード共有はやるが、witness/アンカーは共通時計で代替)。**S4評判の大半・S5法務フル・S7はPhase2に繰り延べ。S6モード分離(Company/Private起動分離)はPhase1で着手可**。`poc/` はS1/S2の参照実装として凍結・保存する。
...
| S3 P2P化 | Company Phase1では"社内版"に縮約(多ノード共有はやるがwitness署名/アンカーは共通時計で代替)。フルP2P(DHT・witness循環対策込み)はPublic Phase2で再拡張 |
