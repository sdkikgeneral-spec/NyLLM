...
| S6 モード分離+UI | Public/Company/Private分離・モード別起動 | 誤操作でPrivateが漏れない | **一部着手**(Company/Private起動分離のみS3実装内で先行実装。UI・Public分離・ゲート判定は未達。注記: 共有キルスイッチ〔CLI `--sharing`+実行中`POST /v1/sharing`、送出5経路を再起動不要で即オフ〕をS3実装の延長として2026-07-20に横断実装=S6ゲート「誤操作でPrivateが漏れない」の趣旨を強める方向・**S6ステータス「一部着手」は不変** → §2 S6節) |
...
**注記(2026-07-21、S4層2/層3〔Public Phase2〕先行設計ノート追加)**: S4ゲート「毒注入テストに耐える」を縮約せず満たすために必要な層2(局所EigenTrust・誕生証明・スラッシング)・層3(消費側抜き打ち再推論・N-of-Mサンドボックス機械検証)の先行設計ノートを [S4_Public_Phase2_層2層3評判機構先行設計.md](./S4_Public_Phase2_層2層3評判機構先行設計.md) として追加した(「骨子+論点構造化」レベルの完成度であり実装可能な仕様書ではない)。層1の実効化にあたっての未解決点(同一モデル問題・自作自演dedup)の整理、既存空スロット(`witness_sigs`/`anchor_proof`/`stake`/`trust.author_reputation`/`trust.revoked`)・5点目のポリシー差替点(`TrustPolicy`)への接続、および「毒注入テストに耐える」ゲートの受け入れ条件を敵対シナリオ(シビル多数決/タグ僭称/検閲/自作自演水増し/国家級APT)×防御機構の対応表として初めて構造化し、合格基準を「Tier-Lクラス別幻覚パリティの非劣化実測+シナリオ1〜4個別の機構実証+層3が国家級APTに対し最後の砦として機能することの確認」という複合条件として提案した。層2はS5(revocation・スラッシング連動)と相互依存であり、witness+アンカー二層は両ノートの共有インフラと位置づける。**本表(§1)のS4ステータス「一部着手」・ゲート「毒注入テストに耐える」は変更しない。**実装着手可否はPublic Phase2移行判断(→§0「移行ゲート」)に委ねる別段階として未着手のまま。
