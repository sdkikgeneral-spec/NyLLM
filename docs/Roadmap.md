...
| lookup時のTTL・クラス別しきい値の強制 **[既知の穴の記録(2026-07-19)。設計原則=「登録時ゲートは情報を不可逆に捨てるため、TTL・クラス別τは検索時に強制する(後からチューニング可能)」。`poc/`のlookupはTTL機構なしのまま凍結(volatileも永久ヒット。PoC意図的縮約)。`src/core`はTTL検索除外(`sync.rs::is_searchable`)を実装済みだがTTL値は暫定(上記S3残課題)で、共有由来エントリへの`SHARED_THRESHOLD`(τ≥0.9)適用が未配線=検索は一律0.80。詳細はArchitecture §7[補完]「既知の穴」、一次記載は信頼性設計メモ §5追記]** | S3(TTL確定チューニング=既存残課題)/ S4以降(クラス別τ・鮮度加重の配線) |
