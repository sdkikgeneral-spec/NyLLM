...
- **[C] `src/core`(Rust)への昇格 = Company Phase1本体** — 背骨(trait 3本/エントリ形式の不変コア/triples/volatility/共有ゲート/検索)を移植。**S3相当を"社内版"に縮約**(多ノード共有はやるが、witness/アンカーは共通時計で代替)。具体設計は [S3_Company_Phase1_社内多ノード共有設計.md](./S3_Company_Phase1_社内多ノード共有設計.md) として確定済み(docs化のみ・実装は本ノート承認後)。**S4評判の大半・S5法務フル・S7はPhase2に繰り延べ。S6モード分離(Company/Private起動分離)はPhase1で着手可**。`poc/` はS1/S2の参照実装として凍結・保存する。
...
現在の最先端(直近で手を付けるべき段階): **S3 P2P化への着手**。S2はゲート(§7フロー通過率を実測)を`bench_pipeline.rs::pipeline_flow_passrate`(代表質問12問の固定セット)で満たし、L2 Agent自己申告/案4トリプル分解/揮発性初期付与(§10.1)/§7フロー判定パイプラインの実装・テスト・脅威モデルレビューを2026-07-17に完了したため「完了」とした(詳細・実測値・残存リスクは下記S2節を参照)。**S3着手前に必須とされていた残存リスク(embedding署名対象化・受信側shareable再導出・provenance署名対象化)はS2.5(エントリ形式再設計)の実装完了(2026-07-17。上記§0[B]参照)により解消済み**。S2.5の脅威モデルレビューでCritical/Highの新規ブロッカーは確認されていないが、情報として記録された残存事項(reload時再導出の非単調性・initial_tier既定値・版フラッディング・state.jsonのノードローカル制約)は [S2.5_エントリ形式設計.md](./S2.5_エントリ形式設計.md) §13 にS3持ち越しとして記録済み。
...
DHT分散・witness署名の実処理・複数版併存の運用ロジックはいずれも `poc/` に該当コードなし。**注記(2026-07-17、S2.5実装後)**: `witness_sigs`/`anchor_proof`/`stake`/`trust` はS2.5の実装により `MutableState` の**空スロット(Phase2予約。常に空Vec/`None`、署名対象外・読み書き未配線)**としては存在するようになったが、これは署名境界を先取り確定するための型のみの予約であり、DHT分散・witness署名・複数版併存というS3の機能自体(中身の実装)は未着手のままである(詳細: [S2.5_エントリ形式設計.md](./S2.5_エントリ形式設計.md) §2「空スロット」)。トップレベル `src/core/`, `src/ui/`(CLAUDE.md記載の将来レイアウト)も未作成(2026-07-17時点でリポジトリに存在しないことを確認)。

**注記(2026-07-17、S3設計ノート追加)**: Company Phase1における社内多ノード共有(中央レジストリ発見+ノード間直接配送・受信側再導出・Company/Private起動分離)の設計は [S3_Company_Phase1_社内多ノード共有設計.md](./S3_Company_Phase1_社内多ノード共有設計.md) として確定した(「Company Phase1設計あり」の意味であり、`src/core` 昇格時に実装対象となる設計ノート)。ただし実装(`src/core` への移植・新規モジュール追加)は本ノート承認後の別段階であり未着手のままのため、本表(§1)のS3ステータス「未着手」は変更しない。同ノートはPublic Phase2固有の機構(DHT/witness/アンカー/ステーク/裁定)を範囲外とし、それらは引き続き未着手・未設計のままである。
...
**注記(2026-07-17、S4層1先行設計ノート追加)**: 3層評判のうち**層1(トリプル一致率によるエントリ内在信頼度)のみを対象とするCompany Phase1先行設計ノード**を [S4_Company_Phase1_層1内在信頼度先行設計.md](./S4_Company_Phase1_層1内在信頼度先行設計.md) として追加した(層2/層3はPublic Phase2の範囲外のまま)。同ノートは「先行設計(docs)・実装は別段階(未着手)」であり、実装はS3(Company Phase1多ノード共有)の実装確定後に依存前提を差分再検証してから着手される。**本表(§1)のS4ステータス「未着手」は変更しない。**
...
| `decompose` の入力長・反復回数上限(DoS対策。受信answerを再分解する設計にする場合) **[脅威レビュー(2026-07-17)。詳細は同上]** | S3(未対応のまま。S2.5の対象外) |
...
| S3(Company Phase1)社内多ノード共有設計(確定) | 中央レジストリ(発見専用)+ノード間直接配送・S2.5 load/verifyの受信側再実行・Company/Private起動時モード分離・組織PKIによるシビル不在の簡約(消すもの/消さないものの線引き)・Public Phase2への非破壊性(ポリシー差し替え点4点)を規定。S2.5 §13の残課題(reload非単調性・tier保守側反転・版フラッディング)との対応も明記。実装は承認後の別段階(未着手) | [S3_Company_Phase1_社内多ノード共有設計.md](./S3_Company_Phase1_社内多ノード共有設計.md) |
