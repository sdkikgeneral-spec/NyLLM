...
- **[B] S2.5 エントリ形式の確定** — 戦略ゲート決着により着手可能(詳細は下記§5参照)。不変コアをPublicまで見据えてフル確定し、可変状態は空スロットを型で宣言する。`poc/` 上・単一ノードのまま。
