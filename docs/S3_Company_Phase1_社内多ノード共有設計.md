...
| 評判/独立検証 | `mutable_state.trust`(None)(**追記 2026-07-19**: うち層1の2フィールド `independent_agreement`/`supporting_versions` はS4層1先行実装でPhase1充填済み — [Architecture.md](./Architecture.md) §8冒頭[補完]・[Roadmap.md](./Roadmap.md) §0対応表S4行が認めた例外。案Bにより `state.json` へは非永続化=起動時にローカル再導出) |
