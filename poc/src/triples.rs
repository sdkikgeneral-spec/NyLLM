...
//  動かしにくい」)。完全なNLPはPoC範囲外であり、「分解成功/失敗」
...
// 事実トリプル(Architecture §6 facts)。CacheEntryに保存され、
// 著者の主張内容そのものなので署名対象に含まれる(cache.rs::signed_payload)。
...
// PoCでは日本語+英語の代表述語のみ(初期構築方法・多言語対応は Roadmap §3 の未解決事項)。
